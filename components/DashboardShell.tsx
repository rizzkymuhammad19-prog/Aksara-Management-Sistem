...
export default function DashboardShell({
  ...
  return (
    <div className="flex">
      ...
      {/* Mobile drawer — always mounted, animated via transform/opacity so it actually slides */}
      <div
      ...
      >
        ...
        <div
          className={`absolute left-0 top-0 h-full w-72 max-w-[85%] shadow-2xl transition-transform duration-300 ease-out ${
        ...
        >
          <div className="relative h-full">
            <button
              ...
              className="absolute right-3 top-4 z-10 text-slate-400 hover:text-white p-1 transition-colors"
      ...
      <div className="flex-1 min-w-0">
        <header className="sticky top-0 z-30 bg-paper/80 backdrop-blur border-b border-slate-200/70 px-4 md:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              ...
              className="md:hidden text-text p-1 -ml-1 active:scale-90 transition-transform"
        ...
        <main key={pathname} className="p-4 md:p-6 animate-fade-in">
          {children}
        </main>
      </div>
    </div>
